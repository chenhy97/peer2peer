...
def p2p_send_file(filename, server_num=1, part_num=1):
	"""send a file with socket by P2P"""
	send_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
	send_socket.connect(ADDR6)

	temp_filename = LIB + filename
	filesize = os.stat(temp_filename).st_size
	...
	with open(temp_filename, 'rb') as file_object:
		#i = 0
		sendsize = 0	#待修改
		while True:
...
class Server(socketserver.BaseRequestHandler):
    ...
    #循环
        ...
        #接收客户发送的数据
            ...
            if cmdname == 'get':
            	rec_address = json.dumps(ADDR6)
            	self.request.send(rec_address.encode())
            	p2p_send_file(filename, server_num, part_num)
            elif cmdname == "quit":
...
if __name__ == '__main__':
    server = socketserver.ThreadingTCPServer(ADDR5, Server) #实现了多线程的socket通话
